...
2. Start generating a prompt.<br>
---Pocess of selecting a token (a single step)---
    1. Take top-k tokens as input. (k is selected manually)<br>
---Divide candidate token (iterate k times)---
    2. Divide the token into Choseong, Jungseong, and Jongseong by unicode arithmeic decomposition.
    ...
    3. Select one of the divided fractions (Choseong, Jungseong, and Jongseong). 
    ...
    4. If this bit matches current step_t of payload, choose token which has this fraction. 
       Increase step_t by 1 and save selected token to an array (token_str)
       If the bit does not match, skip this token. Value of step_t is unchanged.
3. Print generated prompt. (The number of tokens to be printed is designated manually)
4. Conduct detection process and check if the watermark has been successfully inserted.<br>
---Detection---
    1. Take the array of tokens (token_str) of generated prompt as input.<br>
---Inspect each token (iterate as token_str's length)---
    2. If token is Hangul, divide its last syllable into fractions. (Same as watermarking method)
    3. Select one of the divided fractions (Choseong, Jungseong, and Jongseong).
      Find 0 or 1 bit value by a hash equation (order of selecting fraction is also same as watermarking method)
    4. If this value matches with payload's current step_t, this is a fraction of watermarked token.
